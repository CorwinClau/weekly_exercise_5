...
anim_save("national_trains_story.gif")
```  

```{r, echo=FALSE}
knitr::include_graphics("national_trains_story.gif")
```
...
# Ask if the graph should be like this
```
...
```{r, eval=FALSE, echo=FALSE}
anim_save("mallorca_bike_path.gif")
```  

```{r, echo=FALSE}
knitr::include_graphics("mallorca_bike_path.gif")
```
...
# Ask about the second instruction: how to make one single point a different color
# Ask if this needs an animation, for it's not clearly told
```
...
```{r}
# the tick labels look already nice if put both axes on the log scale?
...
covid_anim <- covid19 %>%
  ...
  filter(cases >= 20) %>%
  ungroup() %>%
  ggplot(aes(x = log(cases), y = log(new_cases_week), color = state)) +
  geom_path() +
  geom_point(size = 2.5) +
...
```{r, eval=FALSE, echo=FALSE}
anim_save("covid_density_anim.gif")
```  

```{r, echo=FALSE}
knitr::include_graphics("covid_density_anim.gif")
```
